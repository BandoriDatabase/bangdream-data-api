...
export default async (region) => {
  try {
    const ver = await (await fetch(`${remoteAddr}/bandori-db/public/MAS_VER_${region}`)).text();
    const mdb = await (await fetch(`${remoteAddr}/bandori-db/master/${region}/MasterDB_${ver}.json`)).text();
